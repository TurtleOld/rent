...
from .forms import EpdDocumentForm
...
def upload_epd(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if "account_number" in request.POST:
            ...
            try:
                # Parse the PDF using the parser
...
def edit_epd(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if request.method == "POST":
        form = EpdDocumentForm(request.POST)
        ...
        if form.is_valid():
            ...
            document.total_without_insurance = form.cleaned_data[
